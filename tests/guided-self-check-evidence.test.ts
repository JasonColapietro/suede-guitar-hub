...
import { allowsGuidedSelfCheck } from "../lib/learning/self-check.ts";
import { hasInstructionSelfCheck } from "../lib/learning/instruction-index.ts";
...
test("legacy reflection labels remain compatible only for current guided content", () => {
  ...
  assert.equal(allowsGuidedSelfCheck("guitar", "unrecognized-lesson"), false);
  for (const lessonId of ["g-l1-m1-02", "g-l1-m3-04"]) {
    assert.notEqual(derive(reflection(lessonId))?.assessment, "ready", lessonId);
  }
  const voice = allLessons("voice")[0].lesson;
  assert.equal(derive(reflection(voice.id, { track: "voice" }))?.assessment, "ready");
});
...
test("the compact self-check index matches every current authored lesson on both tracks", () => {
  for (const track of ["guitar", "voice"] as const) for (const { lesson } of allLessons(track)) {
    const instruction = getLessonInstructions(lesson.id);
    const authoredCriteria = !!instruction && instruction.criteria.length > 0 && instruction.criteria.every(criterion => criterion.trim().length > 0);
    assert.equal(hasInstructionSelfCheck(lesson.id), authoredCriteria, lesson.id);
    assert.equal(allowsGuidedSelfCheck(track, lesson.id), authoredCriteria && !lesson.practiceSpec && !instruction?.quiz, lesson.id);
  }
});

test("imported voice range history remains unable to complete any guided lesson", () => {
  const event = reflection("v-l1-m1-01", { track: "voice", kind: "legacy", source: "legacy", disposition: "imported", details: { singSession: { id: "existing-session", durationSec: 60 }, singRangeTest: { lowMidi: 48, highMidi: 72 } } });
  const result = mergeAccountProgress(emptyProgress("voice"), emptyReadingQuizProgress("voice"), [event]);
  assert.deepEqual(result, emptyProgress("voice"));
});
