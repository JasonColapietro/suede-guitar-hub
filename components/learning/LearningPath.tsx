    ...
    {curriculum.levels.map((level, levelIndex) => <details className={styles.level} key={level.id} open={levelIndex === 0}>
      ...
      {level.modules.map((module) => { const available = canOpenModule(track, module.id, access); return <section className={styles.module} key={module.id} aria-labelledby={module.id}>
        ...
        <ol className={styles.lessons}>{module.lessons.map((lesson, index) => {
          ...
          return <li key={lesson.id}><Link className={styles.lessonLink} href={lessonHref(track, lesson.id)}><span className={`${styles.lessonStatus} ${done ? styles.done : ""}`} aria-label={done ? "Marked ready" : `Lesson ${index + 1}`}>{done ? "✓" : index + 1}</span><span><strong>{lesson.title}</strong><span className={`${styles.small} ${styles.muted}`} style={{display:"block"}}>{lessonTypes[lesson.type]}{lesson.practiceSpec ? " · Microphone exercise" : hasInstructionQuiz(lesson.id) ? " · Reading check" : ready ? " · Self-assessment" : " · Preview outline"}{record?.assessment === "repeat" ? " · Revisit" : ""}</span></span><span className={styles.small}>{available && ready ? `${lesson.minutes} min` : ready ? "Preview" : "Outline"}</span></Link></li>;
        })}</ol>
      </section>; })}
    </details>)}
  </>;
