...
function RoutineContent({ accountId }: { accountId: string | null }) {
  ...
  return <div className={styles.routine}>
    ...
    <details className={styles.preparation} open={!prepared}><summary><h2>Before the routine</h2><span>{prepared ? "Preparation available · review or reset" : "Learn the moves, then prepare"}</span></summary><p>Review the GuitarHub instruction first. Saved lesson evidence can satisfy preparation, or confirm that you already understand the exercise. These confirmations describe your preparation, not a skill test.</p>
      <ul className={styles.prepList}>{routineTemplate.preparation.map(item => { const evidence = preparationEvidence(item.id, state, progress); return <li key={item.id}><strong>{item.title}</strong><div className={styles.links}><RoutineLessonLink id={item.lessonIds[0]} pause={pause} /></div>{evidence === "lessonEvidence" ? <p>Available from your saved lesson evidence.</p> : <label className={styles.check}><input type="checkbox" checked={evidence === "selfReported"} disabled={running} onChange={event => { const checked = event.target.checked; change(state => { const preparation = { ...state.preparation }; if (checked) preparation[item.id] = { source: "selfReported", confirmedAt: stamp() }; else delete preparation[item.id]; return { ...state, preparation }; }); }} />I understand this exercise and am ready to practice it.</label>}</li>; })}</ul>
