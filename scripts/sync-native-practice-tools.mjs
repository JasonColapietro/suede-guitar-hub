...
const destination = "contracts/practice-tools.json";
const bytes = await readFile(resolve(sourceRoot, destination));
if (process.argv.includes("--check")) assert.deepEqual(await readFile(destination), bytes, `${destination}: stale native practice tools contract`);
