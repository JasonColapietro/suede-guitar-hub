...
export default async function LessonPage({ params }: { params: Promise<Params> }) {
  ...
  return <>
    ...
    <div className={styles.hero}><p className={styles.small}>{level.stage ? `Stage ${level.stage} · ` : ""}{module.name} · {lesson.minutes} min</p><h1>{lesson.title}</h1>{available ? <p>{lesson.summary}</p> : <span className={styles.badge}>Curriculum preview</span>}</div>
